...
# Firewall rule to allow Bolt connections from VPC (Internal)
...
resource "google_compute_firewall" "neo4j_staging_lb" {
  name    = "allow-neo4j-staging-lb"
  network = google_compute_network.main.name

  allow {
    protocol = "tcp"
    ports    = ["7474", "7687"] # Allow HTTP and Bolt ports
  }
...
resource "google_compute_network_endpoint" "neo4j_staging_endpoint" {
  network_endpoint_group = google_compute_network_endpoint_group.neo4j_staging_neg.name
  instance               = google_compute_instance.neo4j_staging.name
  port                   = google_compute_network_endpoint_group.neo4j_staging_neg.default_port
  ip_address             = google_compute_instance.neo4j_staging.network_interface[0].network_ip
  zone                   = var.zone
  project                = var.project_id
}
  ...
  template {
    ...
    containers {
      ...
      env {
        ...
        value = var.knowledge_admin_channel_staging
      }
...
# Staging Neodash UI
# -----------------------------------------------------------------------------
resource "google_cloud_run_v2_service" "neodash_staging" {
  ...
  template {
    containers {
      ...
      env {
        name  = "standaloneHost"
        value = "neo4j.staging.${var.staging_domain}"
      }
    ...
    # Needs VPC Access to talk to Neo4j Staging if we wanted server-side, 
    # BUT Neodash is client-side. The VPC connector here is only if Neodash backend
...
# Allow LB access
resource "google_cloud_run_v2_service_iam_member" "neodash_staging_public" {
  project  = var.project_id
  location = var.region
